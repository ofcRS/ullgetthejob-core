defmodule Dashboard.CVs.Parser do
  ...
  defp extract_from_pdf(file_path) do
    ...
    case System.cmd("pdftotext", [file_path, "-"], stderr_to_stdout: true) do
      ...
      {error, _} ->
        ...
        {:ok,
         "[PDF file - automated text extraction unavailable. Please install pdftotext or manually copy content.]"}
    end
  rescue
    e ->
  ...
  defp extract_from_docx(_file_path) do
    ...
    {:ok,
     "[DOCX file - automated text extraction not yet implemented. Please convert to PDF or TXT.]"}
  end
end
