defmodule DashboardWeb.CVEditorLive do
  ...
  def handle_event("save_custom_cv", _params, socket) do
    if socket.assigns.job do
      ...
      case Dashboard.Repo.insert(%Dashboard.CVs.CustomCV{} |> Dashboard.CVs.CustomCV.changeset(attrs)) do
        {:ok, custom_cv} ->
          socket =
            socket
  ...
  def handle_info(:generate_suggestions, socket) do
    case CVEditor.suggest_highlights(socket.assigns.cv.parsed_data, socket.assigns.job_requirements) do
      {:ok, suggestions} ->
  ...
  defp is_recommended_skill?(skill, nil), do: false
